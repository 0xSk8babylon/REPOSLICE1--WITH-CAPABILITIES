...
- Audit / Trust Layer plan - docs-only planning approved on 2026-06-25.
  - Canonical plan: `docs/security/AUDIT_TRUST_LAYER_PLAN.md`
  - Current boundary: audit authority remains app-owned and provider-neutral. The plan recommends a future explicit additive Alembic revision for nullable actor references on `audit_events`, a central audit writer, entity-aware `/api/provenance` filtering, privacy-delete audit retention with minimized context, and current-principal consent capture if feasible. No implementation, migration, production DB change, deploy, smoke, Railway wiring, provider SDK, or push has been approved.
  - Deferred: audit runtime service, audit schema migration, provenance filtering implementation, audit query APIs, retention automation, consent actor hardening, contractor/utility grant references, production runtime wiring, and production smoke.
