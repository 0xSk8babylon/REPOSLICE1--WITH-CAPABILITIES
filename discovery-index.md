...
- Address Onboarding backend slice 1 - implemented dedicated address onboarding on 2026-06-25.
  - Canonical plan: `docs/security/ADDRESS_ONBOARDING_PLAN.md`
  - Runtime endpoint: `POST /api/onboarding/address`.
  - Runtime files: `apps/api/app/onboarding/__init__.py`, `apps/api/app/onboarding/router.py`, `apps/api/app/onboarding/schemas.py`, `apps/api/app/main.py`, `apps/api/app/security/auth.py`, and `apps/api/tests/test_address_onboarding.py`.
  - Current boundary: backend-first onboarding route derives or verifies writable account scope from app-owned `account_memberships`, creates only non-null-account homes, returns account-scoped exact minimal normalized address matches, denies viewers/no-writable-account principals, requires explicit `account_id` when multiple writable accounts exist, writes minimized audit context, and creates user-entered address provenance rows for recorded address fields. Demo `account_demo` / `home_001` behavior is preserved.
  - Deferred: frontend onboarding, migrations, external geocoding, postal validation APIs, property/parcel enrichment, utility territory inference, climate/AHJ/program inference, external secrets, provider SDKs, production DB changes, deploy, smoke, Railway wiring, and push.
