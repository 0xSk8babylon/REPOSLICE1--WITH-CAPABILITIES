...
- Ownership / Permissions enforcement slices 1 and 2 - implemented account-membership route filtering on 2026-06-25.
  - Canonical plan: `docs/security/OWNERSHIP_PERMISSIONS_PLAN.md`
  - Runtime files: `apps/api/app/security/permissions.py`, `apps/api/app/security/principal.py`, `apps/api/app/core/repository.py`, `apps/api/app/homes/router.py`, `apps/api/app/buildings/router.py`, `apps/api/app/panels/router.py`, `apps/api/app/loads/router.py`, `apps/api/app/equipment/router.py`, `apps/api/app/designs/router.py`, `apps/api/app/scenarios/router.py`, `apps/api/app/planning/router.py`, `apps/api/app/compatibility_rules/router.py`, `apps/api/app/takeoffs/router.py`, `apps/api/app/design_advisor/router.py`, `apps/api/app/ai_context/router.py`, `apps/api/app/privacy/router.py`, `apps/api/tests/test_permissions.py`, `apps/api/tests/test_privacy.py`, and `apps/api/tests/test_scenario_revisions.py`.
  - Current boundary: active `account_memberships` are the first ownership and authorization boundary. Roles come only from app-owned tables. Implemented filtering covers `/api/homes`, `/api/homes/all`, buildings, panels, loads, load summaries, equipment locations, designs, scenarios, estimated pathways, compatibility issue lists/evaluation, current/generated takeoffs, design advisor summaries, AI design context, design equipment object routes, and scenario revisions. Privacy export/delete are owner-only. `member` is write-capable; `viewer` is read-only; `homes.account_id = null` is hidden/denied by default. Global/reference reads for product library, source documents, rule provenance, load templates, and design goal presets may remain readable to authenticated users for now.
  - Deferred: `home_access_grants`, `home_ownerships`, ownership transfer, contractor/utility/program sharing, app sessions, broader home-path derived route coverage, production auth/runtime wiring, and `data_provenance` entity-aware filtering unless needed earlier.
- Audit / Trust Layer slice 1 - implemented additive audit/provenance foundation on 2026-06-25.
  - Canonical plan: `docs/security/AUDIT_TRUST_LAYER_PLAN.md`
  - Runtime files: `apps/api/app/security/audit.py`, `apps/api/app/security/provenance_access.py`, `apps/api/app/security/auth.py`, `apps/api/app/provenance/router.py`, `apps/api/app/privacy/router.py`, `apps/api/app/services/privacy.py`, `apps/api/app/core/models.py`, `apps/api/migrations/versions/20260625_0003_audit_trust_foundation.py`, `apps/api/tests/test_auth_audit.py`, `apps/api/tests/test_privacy.py`, `apps/api/tests/test_permissions.py`, and `apps/api/tests/test_alembic_foundation.py`.
...
- Address Onboarding backend and frontend slice 1 - implemented dedicated address onboarding on 2026-06-25.
  - Canonical plan: `docs/security/ADDRESS_ONBOARDING_PLAN.md`
  - Runtime endpoint and route: `POST /api/onboarding/address`, frontend `/onboarding/address`.
  - Runtime files: `apps/api/app/onboarding/__init__.py`, `apps/api/app/onboarding/router.py`, `apps/api/app/onboarding/schemas.py`, `apps/api/app/main.py`, `apps/api/app/security/auth.py`, `apps/api/tests/test_address_onboarding.py`, `apps/web/src/pages/AddressOnboardingPage.jsx`, `apps/web/src/pages/HeartQuillShellPage.jsx`, `apps/web/src/app/App.jsx`, `apps/web/src/lib/api.js`, `apps/web/src/components/form/Inputs.jsx`, and `apps/web/src/styles/heartQuillShell.css`.
  - Current boundary: backend-first onboarding route derives or verifies writable account scope from app-owned `account_memberships`, creates only non-null-account homes, returns account-scoped exact minimal normalized address matches, denies viewers/no-writable-account principals, requires explicit `account_id` when multiple writable accounts exist, writes minimized audit context, and creates user-entered address provenance rows for recorded address fields. Frontend shows onboarding only for true no-home states, keeps auth/server errors separate, submits user-entered address fields, and uses `/api/auth/me` account IDs only as a backend-verified account-selection aid. Demo `account_demo` / `home_001` behavior is preserved.
  - Deferred: migrations, external geocoding, postal validation APIs, property/parcel enrichment, utility territory inference, climate/AHJ/program inference, external secrets, provider SDKs, production DB changes, deploy, smoke, Railway wiring, real OAuth wiring, and push.
