...
def list_designs(db: Session = Depends(get_db)):
    return repository.list_designs(db)


@router.post("", response_model=EnergySystemDesign)
def create_design(payload: EnergySystemDesignCreate, db: Session = Depends(get_db)):
    return repository.create_design(db, payload)


@router.patch("/{design_id}", response_model=EnergySystemDesign)
def update_design(design_id: str, payload: EnergySystemDesignUpdate, db: Session = Depends(get_db)):
    if repository.get_design(db, design_id) is None:
...
def list_design_equipment(design_id: str, db: Session = Depends(get_db)):
    if repository.get_design(db, design_id) is None:
        raise HTTPException(status_code=404, detail="Design not found")
...
def create_design_equipment(design_id: str, payload: DesignEquipmentCreate, db: Session = Depends(get_db)):
...
def update_design_equipment(
    design_id: str, equipment_id: str, payload: DesignEquipmentUpdate, db: Session = Depends(get_db)
):
    design = repository.get_design(db, design_id)
    if design is None:
        raise HTTPException(status_code=404, detail="Design not found")
...
def delete_design_equipment(design_id: str, equipment_id: str, db: Session = Depends(get_db)):
    if repository.get_design(db, design_id) is None:
        raise HTTPException(status_code=404, detail="Design not found")
    equipment = repository.get_design_equipment(db, equipment_id)
    if equipment is None or equipment.design_id != design_id:
