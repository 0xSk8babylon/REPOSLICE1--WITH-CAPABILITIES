# Session Handoff Trim Rescue / Consolidation

## Purpose

This is a rescue/consolidation record created during the 2026-07-02
`SESSION_HANDOFF.md` trim. It is not a new implementation session and does not
approve new product, runtime, deployment, migration, push, or architecture
scope.

The content below preserves the pre-trim root `SESSION_HANDOFF.md` snapshot so
historical change-log entries, old verification records, resolved risk items,
superseded restore routing, and any unique details are not lost when the root
handoff is reduced to a current-resume capsule.

## Classification From Trim Pass

- Current-resume-only content kept or summarized in root `SESSION_HANDOFF.md`:
  updated date, branch/state, what changed last, current verification status,
  blockers, next safe action, and the corrected latest-handoff pointer.
- Historical/duplicative content rescued here: long session summaries, canonical
  phase inventories, old "what changed last" entries, earlier change logs,
  historical verification lists, protection/risk registers, superseded restore
  prompts, and the stale June 3 latest-handoff pointer.
- Unique content preserved here rather than dropped: local/production PG-8E
  notes, old commit and phase checkpoints, detailed deferred-boundary language,
  old verification commands and counts, and the Lovable reference parity note
  that had been appended to the root handoff.
- Three handoffs held back from archive during this pass:
  `2026-06-28-lovable-reference-parity-closeout.md`,
  `2026-06-26-planner-intelligence-slice-1-closeout.md`, and
  `2026-06-24-pg8e-local-rehearsal-closeout.md`. They were kept visible because
  they contain the most recent unique operational context for visual parity,
  planner intelligence / production verification, and local PG-8E rehearsal.

## Pre-Trim Root Snapshot
...
- 2026-06-25 Production PG-8E data cutover precheck was documented from Matt's signed-in owner terminal output. The precheck reached production without printing secrets and stopped before dry-run/execute because PG-8E target tables are already non-empty. Production DB connection PASS, `alembic_version=20260523_0001`, expected version match, and `missing_pg8e_tables=[]`. Observed production counts match the known PG-8E migrated baseline for migrated data tables, with `accounts=1`, `homes=1`, `source_documents=5`, `data_provenance=7`, `rule_provenance=25`, `equipment_products=8`, `energy_system_designs=2`, `scenarios=2`, and `audit_events=9`; `audit_events=9` is consistent with prior smoke-created audit-only writes. Record status as PG-8E execute BLOCKED by non-empty target and production effectively already populated pending verification-only closeout, not as a fresh PG-8E execute. No PG-8E execute, truncate, delete, reset, seed, production FastAPI deploy, production FastAPI env-var wiring, public production smoke, Railway resource change, staging change, secret printing, or push occurred. Next immediate step is owner-run read-only PG-8E verification-only closeout from `docs/railway-production-postgres.md`; Production FastAPI deploy/runtime wiring requires separate owner approval.
- 2026-06-25 Production Alembic Gate 2 resumed only. Repo started clean on `fix/github-workflow`, ahead of origin by 39. `scripts/check_production_op_secrets.sh` initially failed inside the sandbox due 1Password daemon/session limitations, then passed outside the sandbox with inherited 1Password access: op auth working, production item exists, and required field titles present. `op run --env-file scripts/templates/production-op.env.tpl` reached the production runtime settings and confirmed `APP_ENV=production`, backend `postgresql`, `DATABASE_CREATE_ALL_ON_STARTUP=false`, and `DATABASE_SEED_DEMO_DATA_ON_STARTUP=false` without printing secrets. Gate 2 stopped before Alembic upgrade because the resolved production URL fields are placeholder-shaped/incomplete and `PGPASSWORD` is placeholder-like; no production schema read/write, Alembic upgrade, PG-8E production migration, seed, production FastAPI deploy/env-var wiring, public production smoke, Railway resource change, staging change, secret printing, push, or production data cutover occurred. Next immediate prerequisite is replacing placeholder/incomplete production values in 1Password, then rerunning Gate 2 only; Production PG-8E data cutover remains a separate later approval after Gate 2 passes.
- 2026-06-24 Residential Energy Planner secrets-management boundary documented as docs-only stabilization work. Added `docs/secrets-management.md` and `.env.example`, linked the boundary from `README.md` and `docs/railway-staging-postgres.md`, and recorded compact state in `PROJECT_STATE.md` and `discovery-index.md`. The runbook covers local development, local disposable Postgres smoke/test DBs, Railway staging, Railway production, CI assumptions, future auth/storage/provider placeholders, intentionally ignored env/data files, and risks/blockers. No real `.env` contents were opened or modified, no Railway resources were created, no migrations were run, no commit was made, and no push was run.
- 2026-06-24 PG-8E local SQLite-to-Postgres rehearsal passed against owned Docker Postgres only. Matt accepted the rehearsal PASS and requested documentation-only closeout. The disposable local target DB was `rep_pg8e_rehearsal`; `rep_pg8_seed_smoke` and `residential_energy_planner` were not written to. The rehearsal reused backup `apps/api/data/backups/residential_energy_planner.pg8e.20260623T070201Z.sqlite3` with checksum `b9cf2b94c3a13b4a66352688a8a19338ef2a6aba86b184d9d218a1e4a4056c9d`. Ephemeral `python:3.11-slim` containers on local Docker network `residential-energy-planner-postgres-dev` ran app requirements, Alembic, the committed `scripts.sqlite_to_postgres` helper, and FastAPI TestClient smoke. `alembic upgrade head` ran only against `rep_pg8e_rehearsal`; `alembic_version` verified `20260523_0001`. Migration dry-run PASS; migration execute PASS; `count_mismatches={}`; source SQLite FK issue count `0`; `data_provenance` and `rule_provenance` source-document orphan counts both `0`. Row-count validation after execute: `accounts=1`, `source_documents=5`, `design_goal_presets=2`, `load_templates=6`, `homes=1`, `audit_events=0` before smoke and `3` after smoke, `consent_records=0`, `buildings=2`, `electrical_panels=1`, `loads=3`, `facts=0`, `roof_planes=0`, `geometry_obstructions=0`, `equipment_locations=3`, `equipment_products=8`, `energy_system_designs=2`, `design_equipment=5`, `compatibility_issues=2`, `scenarios=2`, `scenario_revisions=2`, `takeoff_requests=1`, `takeoff_line_items=2`, `estimated_pathways=2`, `data_provenance=7`, and `rule_provenance=25`. FastAPI smoke against `rep_pg8e_rehearsal` returned `200` for `/`, `/api/accounts`, `/api/homes/all`, `/api/designs`, `/api/scenarios`, and `/api/product-library` with startup create/seed disabled. Sample IDs included `account_demo`, `home_001`, `design_001`, `design_002`, `scenario_001`, `scenario_002`, `product_ecoflow_system`, `product_eg4_hybrid`, `product_enphase_micro`, `source_doc_demo_seed_catalog`, `source_doc_internal_rulebook`, and `source_doc_pathway_walkthrough_note`. No Railway, Supabase, production SQLite, production Postgres, vault, push, frontend Supabase SDK, or repo code change occurred. Repo was clean afterward on `fix/github-workflow`, ahead of origin by 28. `rep_pg8e_rehearsal` remains available for inspection; do not clean it up unless Matt explicitly approves disposable rehearsal DB cleanup.
- 2026-06-15 PG-6B disposable Postgres Home + Fact write/read smoke passed against local database `rep_pg6b_write_smoke`. The fixed Docker subnet was `172.25.0.0/16`, UFW allow rule `[1]` covered `172.25.0.0/16 5432/tcp` before broad deny rule `[48]`, and `alembic upgrade head` ran only against `rep_pg6b_write_smoke`; `alembic current` reported `20260523_0001 (head)`. A fresh one-shot `DATABASE_URL` TestClient process used `x-user-id: pg6b_smoke` and `x-home-access: *`. `POST /api/homes` returned `200` and created `pg6b_home_20260615071345451980`; `GET /api/homes/all` returned `200`, count `1`, and included that home; `POST /api/homes/{home_id}/facts` returned `200` and created `pg6b_fact_20260615071345451980`; `GET /api/homes/{home_id}/facts` returned `200`, count `1`, effective tier `known`, and score `1.0`. `audit_events` contained authorized events for `/api/homes`, `/api/homes/all`, and `/api/homes/{home_id}/facts`. The container was stopped with `docker compose ... down`; no volume deletion, disposable DB drop, downgrade, stamp, migration file, `.env` change, runtime `DATABASE_URL` switch, `.env` Postgres default, FastAPI dev/prod server on Postgres, persistence wiring, sandbox draft persistence, SQLite-to-Postgres migration, production DB use, broader endpoint compatibility approval, commit, or push was performed.
- 2026-06-15 PG-6A-3 test-only FastAPI/TestClient Postgres runtime smoke passed against disposable local Postgres database `rep_pg6a_runtime_smoke`. The fixed Docker subnet was `172.25.0.0/16`, UFW allow rule `[1]` covered `172.25.0.0/16 5432/tcp` before broad deny rule `[48]`, and the DB was already at Alembic `20260523_0001 (head)` before a no-op `alembic upgrade head`. A fresh one-shot `DATABASE_URL` process imported `app.main` after env setup; TestClient `/` returned `200` with redacted URL `postgresql+psycopg://rep_dev:***@127.0.0.1:54329/rep_pg6a_runtime_smoke`, `create_all_on_startup=False`, and `seed_demo_data_on_startup=False`. Authenticated `/api/homes/all` with `x-user-id: pg6a_smoke` and `x-home-access: *` returned `200` with `[]`, and `audit_events` contained an authorized `200` event for `/api/homes/all`. The container was stopped with `docker compose ... down`; no volume deletion, disposable DB drop, downgrade, stamp, migration file, `.env` change, runtime `DATABASE_URL` switch, FastAPI dev/prod server on Postgres, persistence wiring, sandbox draft persistence, SQLite-to-Postgres migration, production DB use, commit, or push was performed.
- 2026-06-15 PG-5A disposable baseline upgrade test passed against local Postgres database `rep_pg5a_baseline_test`. The fixed local Docker subnet is `172.25.0.0/16`, and UFW rule `[1]` allowed `172.25.0.0/16 5432/tcp` before the broad `172.16.0.0/12` deny. The disposable DB was created, outside-sandbox host Python DBAPI and SQLAlchemy `SELECT 1` checks passed, `alembic upgrade head` was run only against `rep_pg5a_baseline_test`, `alembic current` reported `20260523_0001 (head)`, and schema inspection found `alembic_version` plus the 25 SQLAlchemy metadata tables. Codex sandbox Python still has host-networking limitations and failed the same DBAPI/SQLAlchemy host checks, but outside-sandbox host Python passed. The container was stopped with `docker compose ... down`; no volume deletion, disposable DB drop, downgrade, stamp, migration file, `.env` change, runtime `DATABASE_URL` switch, FastAPI startup against Postgres, persistence wiring, production DB use, commit, or push was performed.
- 2026-06-15 PG-5 connection-only local Postgres smoke is recorded as resolved/passed after manual host firewall correction. Root cause was a UFW broad outbound DROP for `172.16.0.0/12` blocking Docker bridge traffic; the manual allow rule `allow out to 172.25.0.0/16 port 5432 proto tcp`, ordered before the DROP, allowed DBAPI and SQLAlchemy `SELECT 1` smoke checks to pass. A later Codex rerun timed out when Docker allocated a different subnet, so PG-4B pinned the local Compose network to `172.25.0.0/16`.
...
- Verification passed: `git diff --check`; focused Phase 11 tests `8 tests OK` in about 141s.
...
## Lovable Reference Parity Pass (2026-06-27)

- Stable reference set: `~/TwinEnergy/references/lovable-references/` (home/explore/planner/builder/capabilities-unsorted). Visual/card parity evidence only — not implementation authority, not backend truth.
- Authority order: owner-workflows > current repo code/contracts > heart-quill source repo > Lovable screenshots. Screenshots must not override owner-workflows or current repo contracts.
- `capabilities-*` are "Product Objects" derived-view references → classification evidence only, kept on internal/debug Capabilities surface, never promoted to primary nav.
- Planner: 2 backend templates are the only finalized ones; the 3 mock-only templates are render-only drafts (`isDraft`, `source:"mock"`, reduced opacity, "Draft" badge, selection disabled, guarded). Not ported to backend, never POSTed/validated.
- Home: empty Postgres is valid; 404 `/api/homes` renders full shell + additive onboarding (not a substitutive single card). 401 stays an access warning. No default seeding.
- Protocol + parity table: `docs/lovable-reference-parity.md`.
