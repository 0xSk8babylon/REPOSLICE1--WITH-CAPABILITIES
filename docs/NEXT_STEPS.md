...
Preserve and use the new repository cognition structure as the restore and governance substrate, then resume the deeper solar-readiness slice with better internal roof-capacity and placement realism while preserving the current additive profile architecture, explicit current-state solar topology, explicit backup scope, inverter/system architecture reasoning, structured dependency traceability, and planning-only trust boundary ahead of any final inverter sizing.
...
4. Use `docs/security/SCOPED_VIEW_MODEL_MAPPING.md` as the contract-design map before implementing RBAC, exports, utility-facing packets, contractor-facing packets, or operational-control views; current additive metadata and mapping only label boundaries.
5. If scoped-view work resumes, start with an additive `AIDesignGroundingView` candidate rather than narrowing the existing broad AI context endpoint.
