...
- Address onboarding backend slice 1 adds `POST /api/onboarding/address` as an additive first-time home creation/resolution workflow. The frontend slice adds `/onboarding/address` and links the Home shell true no-home state to that route. The combined slice does not add external geocoding, address validation APIs, property enrichment, utility inference, climate/AHJ/program inference, provider SDKs, secrets, migrations, production wiring, or real OAuth behavior.
