...
- Audit/trust layer planning is documented in `docs/security/AUDIT_TRUST_LAYER_PLAN.md`. Future audit authority should come from the app-owned principal, account memberships, route/object authorization decisions, and source/provenance records, never provider claims. First implementation is expected to be additive and to preserve existing `audit_events` fields for compatibility.
...
- Planned audit/trust implementation should add stronger app-owned audit actor references and entity-aware `/api/provenance` filtering through a future explicit migration and route changes. No new audit query API is approved yet.
