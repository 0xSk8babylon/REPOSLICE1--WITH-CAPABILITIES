...
## Provenance Access Contract

- `GET /api/provenance` now requires the current app principal.
- Data provenance records are filtered by app-owned access to the referenced entity.
- Direct home-scoped and design/scenario-scoped entity references are allowed only when the principal can access the underlying home/account scope.
- Intentional global/reference entity types such as product, source-document, rule-provenance, and design-goal references may remain readable to authenticated users.
- Unknown provenance entity types are omitted by default.
