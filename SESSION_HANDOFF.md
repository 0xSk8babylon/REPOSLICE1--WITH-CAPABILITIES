...
- Current Phase 8 checkpoint:
  - Phase 8 is defined as Topology Takeoff & Material Cost Engine. Phase 8 is complete in the working tree and not yet staged or committed; no push was run. Phase 8 adds backend read-only schemas, service logic, and route support for `GET /api/twin-planning-context/homes/{home_id}/views/topology-takeoff`. The view derives planning-grade material/scope categories from existing topology and planning context, including PV source circuit/array-side scope, inverter/power electronics, battery/ESS, backup interface/gateway/transfer, generator integration, panel/subpanel/load center, conduit/raceway pathway, conductor/circuit placeholders, disconnect/OCPD placeholders, monitoring/communications, labeling/signage, grounding/bonding, and routing/trenching/structural/mounting where supported by current data. Every line includes traceable basis/provenance, basis quality, quantity-basis posture, cost-basis-unavailable metadata, uncertainty, missing information, blockers, confirmation gates, and homeowner-safe/contractor-facing interpretation metadata. Runtime files changed are `apps/api/app/twin_planning_context/schemas.py`, `apps/api/app/twin_planning_context/router.py`, `apps/api/app/services/twin_planning_context.py`, and `apps/api/tests/test_twin_planning_context.py`. No persistence, migrations, write endpoints, auth/security changes, permission enforcement, frontend work, exports, pricing/proposals, final estimate, final bill of materials, contractor-approved BOM, final wire/conduit/breaker sizing, final disconnect/OCPD approval, permit-ready design, AHJ/utility approval, field-verification approval, graph behavior, `twin_id`, operational behavior, or final electrical design output was introduced. Verification passed: focused Phase 8 tests `7 tests OK`; focused Phase 7 regression tests `6 tests OK`; full twin planning context backend tests `155 tests OK`.
...
- Phase 8: Topology Takeoff & Material Cost Engine - complete in the working tree as `GET /api/twin-planning-context/homes/{home_id}/views/topology-takeoff`.

## What Changed Last

- Phase 7 Shared Compatibility & Install Path View was implemented in the working tree and is not yet staged or committed.
- Added backend read-only shared compatibility schemas on `apps/api/app/twin_planning_context/schemas.py`.
- Updated `apps/api/app/services/twin_planning_context.py` with request-time derived classification over existing `TwinPlanningContext`, topology/readiness outputs, Phase 5 confirmation gates/install complexity signals, and the Phase 6 Planning Exchange Object.
- Updated `apps/api/app/twin_planning_context/router.py` with `GET /api/twin-planning-context/homes/{home_id}/views/shared-compatibility`.
- Updated `apps/api/tests/test_twin_planning_context.py` with focused coverage for route presence, status classification, basis/provenance, missing information, contractor confirmation gates, trust boundaries, deterministic same-input/same-output behavior, and absence of final-design claims.
- Verification passed: focused Phase 7 tests `6 tests OK`; full twin planning context backend tests `148 tests OK`; `git diff --check` passed.
- No frontend, persistence, migration, auth/security, permission enforcement, write endpoint, export/PDF/share link, pricing/proposal, final wire/conduit/breaker sizing, final disconnect/OCPD approval, permit-ready design, AHJ/utility approval, field-verification approval, `twin_id`, graph, operational behavior, push, or source-of-truth mutation work has been performed during Phase 7.
