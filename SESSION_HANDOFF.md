...
- 2026-06-15 PG-5 connection-only local Postgres smoke is recorded as resolved/passed after manual host firewall correction. Root cause was a UFW broad outbound DROP for `172.16.0.0/12` blocking Docker bridge traffic; the manual allow rule `allow out to 172.25.0.0/16 port 5432 proto tcp`, ordered before the DROP, allowed DBAPI and SQLAlchemy `SELECT 1` smoke checks to pass. Codex rerun confirmed the container starts healthy and publishes `0.0.0.0:54329->5432/tcp`; the rerun timed out because Docker recreated the bridge on `172.26.0.0/16`, outside the manually allowed subnet. The container was stopped with `docker compose ... down`; no volume deletion, migration upgrade, downgrade, stamp, migration file, `.env` change, runtime `DATABASE_URL` switch, persistence wiring, commit, or push was performed. The next boundary is PG-5A Disposable Baseline Upgrade Test planning, before any runtime switch or persistence planning.
